...
import { generateErrorView, generateLayoutView, generateLoginView, generateRegisterView } from './views.js';

export function generateStructure(config) {
  const { template, useRoutes, useControllers, useMongoDB, useMySQL, useAuth } = config;
  const templateConfig = template !== 'none' ? htmlTemplates[template] : null;
